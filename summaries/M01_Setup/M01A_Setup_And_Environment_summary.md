...
- **Virtual environment (venv)**
  - **Term:** A self-contained Python installation where you can install specific package versions without affecting anything else on your computer. The course creates one called `.venv` inside the `openai-agents` folder.
  ...
  - **What students need to understand:**
    - You must *activate* the venv (`source .venv/bin/activate` on Mac/Linux, `.\.venv\Scripts\Activate.ps1` on Windows) before installing packages or registering the kernel
    - The packages are defined by `requirements.txt` — that file pins every version so everyone has the same setup
    - If packages are missing later, `pip install -r requirements.txt` brings you back to the correct state
    - In VS Code, the kernel name `openai-agents` (registered via `ipykernel`) is how you confirm the right environment is selected
...
- **`sys.executable` / `sys.prefix`**
  ...
  - **What students need to understand:**
    ...
    - If neither path contains `"openai-agents"`, you likely launched JupyterLab before activating the venv
    - You don't need to memorize these variables; the notebook does the check for you

- **`requirements.txt`**
  - **Term:** A file that lists every package and version the course needs. Pip reads it and installs everything in one shot.
  - **Analogy:** It's a recipe card — hand it to pip and it assembles the exact same pantry every time, on any machine.
  - **What students need to understand:**
    - Students should have already created the venv and installed from this file in the setup lecture before opening this notebook
    - `pip install -r requirements.txt` (with the venv active) is the fix-all command when something is missing
...
try:
    ...
    print("❌ openai-agents missing — run: pip install -r requirements.txt")
    all_ok = False
```
...
**Check 1: Python Version.** This cell imports `sys` and reads `sys.version_info.major` and `sys.version_info.minor` to confirm Python 3.10+. On camera, emphasize that the check is looking for *at least* 3.10 — the venv installs whatever Python version was used to create it (3.11+ for this course), so students will see something like "Python 3.11 detected." If someone sees 3.9 or lower, it means they didn't activate the venv. Point out the helpful error message that says exactly what to do: `source .venv/bin/activate` (Mac/Linux) or `.\.venv\Scripts\Activate.ps1` (Windows).
...
- **Students may not have run the setup lecture yet.** This notebook *assumes* the venv already exists and the packages are installed. If a student jumps straight here, every check will fail. On camera, say: "If you haven't watched the setup lecture and created the venv yet, do that first — this notebook checks your work, it doesn't do the setup for you."
